...
class Trial:
    ...
    Output layout:
      <base_output_dir>/baseline/batch_YYYYmmdd_HHMMSS/
        ...
        - baseline_summary_complete_allruns.csv
        - baseline_queue_ed_assessment.csv
        - baseline_queue_medical.csv
        - baseline_queue_consultant.csv
        - baseline_queue_amu.csv
        - baseline_ed_doctor_blocks.csv
        - baseline_resource_monitor.csv
    ...
    def __init__(self, global_params, master_seed, base_output_dir=None):
        ...
        # Default to <project_root>/output/des_output unless caller overrides
        self.base_output_dir = os.path.abspath(
            base_output_dir
            or os.path.join(os.path.dirname(os.path.dirname(__file__)), "output", "des_output")
        )
    ...
    def run(self, run_number: int, progress_bar=None):
        ...
        # Return handles for programmatic inspection
        return {
