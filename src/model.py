...
class Model:
    ...
    def __init__(self, global_params, burn_in_time, run_number):
        ...
        assert {"p_cal", "p_raw", "weight"} <= set(self.admission_probability_distribution_data.columns), \
            "Expected columns p_cal, p_raw and weight in the distribution CSV."

        # Aggregated empirical distribution of (raw, calibrated) referral probabilities.
        # Each row is a rounded (p_raw, p_cal) bin with `weight` = number of patients it
        # represents. Patients sample a bin in proportion to its weight (see below),
        # which reproduces the original per-patient distribution without storing
        # patient-level rows.
        ...
        # Normalised cumulative weights for inverse-CDF sampling (one uniform draw per patient).
        _w = self.admission_probability_distribution_data["weight"].to_numpy().astype(float)
        self._p_cumweights = np.cumsum(_w)
        self._p_cumweights /= self._p_cumweights[-1]
       ...
       # --- Buffers (fast) ---
        ...
        self.amu_queue_df = pd.DataFrame(columns=["Time","Queue Length"])
    ...
    def generate_arrivals(self):
        ...
        while True:
            ...
            if adult:
                # Weighted draw from the aggregated distribution via inverse CDF
                # (one uniform per patient, equivalent to uniformly picking a patient row).
                u = self.rng_probs.random()
                idx = int(np.searchsorted(self._p_cumweights, u, side="right"))
                if idx >= self._p_n:
                    idx = self._p_n - 1
    ...
    def obstruct_triage_nurse(self):
        """Simulate increased triage nurse capacity in peak hours 12:00 and 22:00."""
        while True:
            # Extract the current hour
            current_hour = extract_hour(self.env.now)
    ...
    def consultant_assessment(self, patient):
        ...
        with self.consultant.request(priority = patient.priority) as req:
            ...
            # Discharge after consultant assessment logic
    ...
    def run(self):
        ...
        # Start monitoring the ED assessment queue
        ...
        # Start the medical doctor obstruction process
